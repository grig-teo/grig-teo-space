...
export function Experience({ items }: { items: ExperienceItem[] }) {
  ...
  return (
    <section id="experience" className="px-4 py-12 sm:px-6 sm:py-16 md:px-12">
      <h2 className="mb-8 text-lg sm:mb-10">{t('experience')}</h2>
      <div className="relative max-w-4xl">
        <div className="absolute left-[5px] top-2 bottom-2 w-px bg-border" />
        <div className="space-y-8 sm:space-y-10">
          {items.map((item) => (
            <div key={item.id} className="grid grid-cols-[12px_1fr] gap-x-4 sm:gap-x-6">
              ...
              <article>
                <div className="mb-1 flex flex-col gap-1 md:flex-row md:items-baseline md:gap-4">
                  <time className="text-xs text-muted md:min-w-[120px]">{item.period}</time>
